...
/*
    ...
    이상에서 중요한 점은
    클래스명.매서드명() 이라는 점입니다.
    scanner.nextLine();의 경우에는 객체명.메서드명()이 었는데 차이가 있습니다.
    그리고 toString()이라는 메서드도 곧 사용하게 됩니다.

    이상에서 확인해야 할 부분은 동일한 method 명이라고 하더라도
    어떤 클래스에 종속되어 있는가에 따라 다른 결과값을 지닐 수 있다는 점입니다.
 */
public class Array10PrintArray {
    public static void main(String[] args) {
        ...
        // 어떤 클래스의 객체에서 호출하느냐에따라서 결과값이 다르다.
...
//        System.out.println(Arrays.toString(sort(nums)));
        // 이상의 코드는 오류가 난다. 먼저 바꿀건지 먼저 출력할건지 모른다.
        // Arrays.sort(nums);의 결과값이 void기 때문에 return이 없는데
        // Arrays.toString();에는 배열을 넣어야만 하기 때문에 오류가 발생한다.

        // method overloading의 예시라고 볼 수 있다 매개변수가 2개로 늘어남
        Arrays.sort(nums, Comparator.reverseOrder()); // 현재 오류가 발생함 int[]
        // nums를 Integer[]로 바꾸니까 오류가 사라졌다.
