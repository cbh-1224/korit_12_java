...
//        animal2.fetch();     // 불가능
        /*
            ...
            1. Override가 적용된 메서드, 즉 자식 클래스의 메서드가 호출됨(클래스 B의 메서드라는 의미)
        ...
        System.out.println("------------------");
        // : 업캐스팅이 이러우지지 않고 Dog dog1 = new Dog();로 B b = new B(); 단 B는 A의 자식 클래스인 경우
        ...
        // 위에 Animal animal2 = new Dog();로 생성했기 때문에 animl2의 Dog버전 객체명이 아예 없었다.
        // 근데 이번에 다운캐스팅하면서 dog2라고 하는 객체명을 지어줬다고 할 수 있다.
        animal2.makeSound();
        dog2.makeSound();
        dog2.fetch();
        /*
...
//        dog3.makeSound();
        /*
            이상의 경우를 확인했을 때 dog3.makeSound();로 가기 전에 이미 downcasting 부분에서(77번 라인) 오류가
            발생했음을 확인할 수 있습니다.
        ...
        // 위와 같은 방식으로 선언된 클래스와 호출되는 생성자가 서로 달라야 한다.(그리고 둘은 상속관계여야만 한다.)

        // 근데 문제는 빨간 줄 미리 띄워서 오류가 발생할 예정이라고 알려주지 않는다.
