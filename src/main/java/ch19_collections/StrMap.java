...
public class StrMap {
    public static void main(String[] args) {
        ...
        // strMap1의 자료형은 당연히 Map일 텐데 .entrySet()의 호출 결과가 Set<Map.Entry<String, String>
        // 으로 자동 완성 된 이유는 저희가 그렇게 선언했기 때문입니다. -> intellij support에 해당함
        // 그럼 이제 우리수준에서는 strMap1.entrySet();의 호출결과가 일종의 형변환이 이루어진 로직이 있꼬
        // 해당 자료형으로 return해서 entrySet2라는 객체에 데이터를 담았다고 해석할 수 있다.
        System.out.println(entrySet2);

        // 근데 Map을 Set으로 바꿨다면 아까 학습한 내용을 토대로 봤을 때 Set -> List로 형변환할 수 있겠네요.
...
//        Set<String> valueSet = strMap1.values();
        /*
            ...
            각 Key의 Value들은 중복 값이 있을 수도 있기 때문에 그것을 Set 자료형으로
            변경할 경우 데이터 누락이 생길 수 있으므로 불가능합니다.
         */
        Collection<String> values = strMap1.values();
        System.out.println(values);
        /*
            Collenctions는 .sort()할 때 써봤지만 Collection은 처음 써봣습니다.
            근데 return 형 본 다음에 직접 직접 쓰신다면 너무 좋겠네요
         */
